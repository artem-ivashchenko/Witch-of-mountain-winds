...
shopTabs.forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    const categoryProd = document.querySelector(`
      .shop__product[data-category="${tab.dataset.category}"]`);
...
inputsbox.forEach(inputbox =>
  inputbox.children[0].addEventListener('input', () => {
    ...
    inputsbox.forEach(inputbox => {
      ...
      if (!input.value) {
        ...
        if (!test) {
          ...
          inputbox.children[1].textContent = `
            Phone must have +380XXXXXXXXX format
          `;
          error = true;
        }
      }
